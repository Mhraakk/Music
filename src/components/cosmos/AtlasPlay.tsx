...
import type { AtlasNestId } from "@/lib/atlas/nest";
...
export function AtlasPlay({
  ...
  async function play() {
    ...
    setNote(null);
    try {
      const feelings = surface === "feelings" || Boolean(feeling);
      const response = await fetch(feelings ? "/api/feelings/harvest" : "/api/atlas/harvest", {
        method: "POST",
