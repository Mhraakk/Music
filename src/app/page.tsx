...
export default function App() {
  ...
  const fbKey = useMemo(
    () => Object.entries(by).map(([k, v]) => `${k}:${v.kind}:${v.reason || ""}`).sort().join("|"),
    [by]
  );

  const res = useMemo(() => orchestrateRecommendations(c, by, depth, { limit: 12 }), [c, by, depth, fbKey]);
  const fl = useMemo(() => buildJourneyPlan(c, by, depth, 6), [c, by, depth, fbKey]);
  const g = useMemo(() => graph(by), [by, fbKey]);

  if (typeof window !== "undefined") {
    (window as any).__RESONANT__ = { clearRecent, getDebugSnapshot, recommend, flow, lastRecommend: res };
  }
  ...
  return (
    <div className="relative min-h-dvh">
      ...
      <div className="relative z-10 mx-auto max-w-lg px-4 pt-8 pb-44">
        <header className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-2xl flex items-center justify-center text-sm font-bold tracking-tight" style={{ background: "linear-gradient(145deg, #e8a06a, #8b3a1a)", boxShadow: "0 0 28px rgba(232,160,106,0.4), inset 0 1px 0 rgba(255,255,255,0.2)", color: "#1a0e08" }}>R</div>
            <div>
        ...
        {tab === "graph" && (
          <>
            <h1 className="mt-8 text-[1.85rem] font-semibold tracking-tight text-[#f8f4ee] leading-[1.15]">Listen by feeling,<br /><span className="text-white/38">not by genre.</span></h1>
            <p className="mt-2.5 text-[14px] text-white/38 leading-relaxed max-w-[22rem]">Artwork, atmosphere, rejection memory — recommendations that pass an emotional test.</p>

            {res.items[0] && (
              <button type="button" onClick={() => play(res.items[0].t, queue)} className="mt-6 w-full text-left glass-3 glass-edge rounded-[1.5rem] overflow-hidden pressable group" style={{ boxShadow: `0 24px 60px rgba(0,0,0,0.45), 0 0 40px rgba(${res.items[0].t.ambient}, 0.15)` }}>
                ...
                <div className="p-4"><p className="text-[12.5px] text-[#d4a574]/95 leading-relaxed">{res.items[0].reason}</p></div>
              </button>
            )}
            ...
            <section className="mt-5 glass-2 glass-edge rounded-[1.25rem] p-4 space-y-3.5">
              ...
              <Slider label="Discovery temperature" value={temp} onChange={(n) => { setTemp(n); setTemperature(n); setPortalItems(null); }} />
            </section>

            <div className="mt-4">
              <DiscoveryPortals compass={c} fb={by} depth={depth} onResult={(items, meta) => {
                setPortalItems(items); setPortalTitle(meta.title);
            ...
            <div className="mt-3 glass-1 rounded-xl px-3.5 py-2.5 text-[11px] text-white/50 leading-relaxed">
              <span className="text-[#e8a06a]/90">Taste Twin · </span>{Math.round(twin.howWellIKnowYou * 100)}% known — {twin.summary}
            </div>

            <div className="mt-3 glass-1 rounded-xl px-3.5 py-2.5 text-[11px] text-white/50 leading-relaxed">
              <span className="text-[#e8a06a]/90">Graph · </span>{res.graph?.voice}
              {res.graph?.avoids && res.graph.avoids.length > 0 && (<span className="text-white/30"> · avoids {res.graph.avoids.slice(0, 3).join(", ")}</span>)}
            </div>

            <HealthBanner tier={res.tier} health={res.health} message={portalItems ? null : res.message} poolSize={res.metrics?.poolSize} itemCount={displayItems.length} correlationId={res.correlationId} sources={res.sources} />

            <h2 className="mt-7 text-[10px] uppercase tracking-[0.15em] text-white/35">{portalTitle ? portalTitle : "Passes emotional test"}</h2>
            {portalItems && (
              <button type="button" className="text-[10px] text-white/40 mt-1 pressable" onClick={() => { setPortalItems(null); setPortalTitle(null); }}>clear portal · back to graph rank</button>
            )}
            <div className="mt-2.5 space-y-3">
              {displayItems.map(({ t, reason }, i) => (
                <TrackCard key={t.id + (portalTitle || "")} track={t} reason={reason} variant={i === 0 ? "editorial" : "discovery"} queue={queue} />
              ))}
            </div>
          </>
        )}
        ...
        {tab === "flow" && (
          <>
            ...
            <div className="mt-4"><MusicalGps compass={c} path={fl.path.map((p) => p.t)} /></div>
            {twin.sessionNarrative && <p className="mt-3 text-[12px] text-white/45 leading-relaxed">{twin.sessionNarrative}</p>}
            ...
            {(() => {
              ...
              return (
                <div className="mt-5 glass-2 glass-edge rounded-2xl p-4 space-y-2">
                  ...
                  <p className="text-[12px] text-white/55">Long-term samples {dna.longTermSamples} · medium {dna.mediumSamples} · session {dna.sessionSamples}</p>
                  <p className="text-[12px] text-white/45">Negative strength {Math.round(dna.negativeStrength * 100)}%{dna.negativeArtists.length > 0 && <span> · avoids {dna.negativeArtists.slice(0, 4).join(", ")}</span>}</p>
                </div>
              );
            })()}
            ...
            <div className="mt-6 space-y-3">
              {([["Dark", g.attract.d],["Warm", g.attract.w],["Organic", g.attract.o],["Energy", g.attract.e],["Sad", g.attract.s],["Mainstream pull", g.attract.m]] as [string, number][]).map(([label, v]) => (
                <div key={label} className="space-y-1">
                  ...
                  <div className="h-1.5 rounded-full bg-white/5 overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-[#6b3a18] to-[#e8a06a]" style={{ width: `${Math.round(v * 100)}%` }} /></div>
                </div>
              ))}
            </div>
      ...
      <MiniPlayer />
      <NowPlaying />
      <AgentPanel compass={c} depth={depth} tab={tab} onCompass={(partial, d) => { setC((prev) => ({ ...prev, ...partial })); if (typeof d === "number") setDepth(d); }} onTab={setTab} />
      ...
      <nav className="fixed bottom-0 inset-x-0 z-30 border-t border-white/6 bg-black/65 backdrop-blur-2xl pb-safe">
        <div className="mx-auto max-w-lg flex justify-around py-3.5">
          {(["graph", "flow", "self"] as const).map((t) => (
            <button key={t} type="button" onClick={() => setTab(t)} className={`text-[11px] font-medium capitalize tracking-wide pressable px-4 py-1.5 rounded-full transition-colors ${tab === t ? "text-[#e8a06a] bg-white/5" : "text-white/32"}`}>{t}</button>
          ))}
        </div>
      </nav>
    </div>
