  ...
  return (
    <div className="relative min-h-dvh">
      ...
      <div className="relative z-10 mx-auto max-w-lg px-4 pt-8 pb-44">
        ...
        {tab === "graph" && (
          <>
            <h1 className="mt-8 text-[1.85rem] font-semibold tracking-tight text-[#f8f4ee] leading-[1.15] display">
              Listen by feeling,
              <br />
            ...
            {portalItems && (
              <button
                ...
                onClick={() => { setPortalItems(null); setPortalTitle(null); }}
              >
                clear portal · back to graph rank
              </button>
            )}
            <div className="mt-2.5 space-y-3 motion-stagger">
              {displayItems.map(({ t, reason }, i) => (
                <TrackCard
                  key={t.id + (portalTitle || "")}
                  track={t}
                  reason={reason}
              ...
              return (
                <div className="mt-5 glass-2 glass-edge rounded-2xl p-4 space-y-2">
                  ...
                  <p className="text-[12px] text-white/55">
                    Long-term samples {dna.longTermSamples} · medium {dna.mediumSamples} · session {dna.sessionSamples}
                  </p>
                  ...
                  {dna.recentSkips.length > 0 && (
                    <p className="text-[11px] text-white/35">
                      Recent skips: {dna.recentSkips.map((s) => s.kind).join(" · ")}
                    </p>
                  )}
                </div>
              );
