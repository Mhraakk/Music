  ...
  switch (id) {
    case "deeper": {
      ...
      return {
        id,
        title: "Go Deeper",
        description: "Farther from the obvious, still connected to your room",
        items: res.items.map((x) => x.t),
        depth: d,
        temperature: 0.45 + d * 0.4,
      };
    }
    case "stranger": {
      setTemperature(0.85);
      ...
      return {
        id,
        title: "Stranger",
        description: "Controlled surprise — low probability, hidden link",
        items,
        depth: Math.min(1, depth + 0.25),
        temperature: 0.85,
      };
    }
    case "older": {
      ...
      const merged = [
        ...older,
        ...res.items.map((x) => x.t).filter((t) => !older.find((o) => o.id === t.id)),
      ].slice(0, 8);
      return {
        id,
        title: "Older",
        description: "Pull toward earlier catalog gravity",
        items: merged,
      };
    }
    case "future": {
      ...
      return {
        id,
        title: "Future",
        description: "Later descendants of your current texture",
        items: newer.length ? newer : recommend(compass, fb, depth).items.map((x) => x.t),
      };
    }
    case "same_feeling": {
      const c: Compass = {
        ...
        organic:
          compass.organic > 0.5
            ? Math.max(0.15, compass.organic - 0.35)
            : Math.min(0.85, compass.organic + 0.35),
      };
      const res = recommend(c, fb, depth);
      return {
        id,
        title: "Same Feeling / Different Genre",
        description: "Emotional structure held — texture world changed",
        items: res.items.map((x) => x.t),
        compass: c,
      };
    }
    case "opposite_energy": {
      const c: Compass = {
        ...compass,
        energy: 1 - compass.energy,
        dark: Math.min(1, compass.dark + (compass.energy > 0.5 ? 0.1 : -0.05)),
      };
      const res = recommend(c, fb, depth);
      return {
        id,
        title: "Opposite Energy",
        description: "Invert pulse while keeping the rest of the room",
        items: res.items.map((x) => x.t),
        compass: c,
      };
    }
    case "reset_room": {
      ...
      return {
        ...
        description: "Session bias cleared path — open catalog center",
        items: res.items.map((x) => x.t),
        compass: { warm: 0.55, sad: 0.45, organic: 0.5, energy: 0.35, dark: 0.55 },
        depth: 0.55,
        temperature: 0.45,
      };
    }
    default:
      return {
        ...
        title: "Go Deeper",
        description: "",
        items: recommend(compass, fb, depth).items.map((x) => x.t),
      };
  }
}
