...
export type TasteDNA = {
  ...
  medium: DnaLayer;
  session: DnaLayer;
  negative: NegativeDna;
  skips: { trackId: string; kind: SkipKind; at: number }[];
  version: 1;
};
...
function emptyDna(): TasteDNA {
  ...
  return {
    ...
    medium: layer(0.3),
    session: layer(0.2),
    negative: { artists: {}, emotions: {}, repel: { ...ZERO }, strength: 0 },
    skips: [],
    version: 1,
  };
}
function loadDna(): TasteDNA {
  ...
  try {
    ...
    return { ...emptyDna(), ...JSON.parse(raw) } as TasteDNA;
  } catch {
    return emptyDna();
  }
}
let dnaCache: TasteDNA | null = null;
export function getTasteDNA(): TasteDNA {
  if (!dnaCache) dnaCache = loadDna();
  return dnaCache;
}
function persistDna() {
  if (typeof window === "undefined" || !dnaCache) return;
  try {
    localStorage.setItem(DNA_KEY, JSON.stringify(dnaCache));
  } catch {
    /* */
  }
}
...
export function applyDecay(now = Date.now()): TasteDNA {
  ...
  if (medAge > 7) {
    d.medium.attract = blendVec(d.medium.attract, ZERO, Math.min(0.4, medAge / 30));
    d.medium.weight = Math.max(0.15, d.medium.weight * 0.95);
  }
...
export function recomputeDnaFromFeedback(fb: FB): TasteDNA {
  ...
  if (liked.length) {
    ...
    d.longTerm.sampleCount += liked.length;
    d.longTerm.updatedAt = Date.now();
    d.medium.attract = blendVec(d.medium.attract, m, 0.45);
    d.medium.sampleCount = liked.length;
    d.medium.updatedAt = Date.now();
  }
  const session = getSession();
  const sessionTracks = session.recentTrackIds
    .map((id) => TRACKS.find((t) => t.id === id))
    .filter(Boolean) as Track[];
  if (sessionTracks.length) {
    d.session.attract = meanOf(sessionTracks);
    d.session.sampleCount = sessionTracks.length;
    d.session.updatedAt = Date.now();
  }
  const artists: Record<string, number> = {};
  const emotions: Record<string, number> = {};
  for (const t of hated) {
    const a = t.artist.toLowerCase();
    artists[a] = Math.min(1, (artists[a] || 0) + 0.35);
    emotions[t.emotion] = Math.min(1, (emotions[t.emotion] || 0) + 0.25);
  }
  d.negative.artists = artists;
  d.negative.emotions = emotions;
  if (hated.length) {
    d.negative.repel = meanOf(hated);
    d.negative.strength = Math.min(1, hated.length * 0.2);
...
export function compositeAttract(fb: FB, compass: Compass): Vec {
  applyDecay();
  recomputeDnaFromFeedback(fb);
  const d = getTasteDNA();
  const g = graph(fb);
  let v = blendVec(d.longTerm.attract, d.medium.attract, 0.4);
  v = blendVec(v, d.session.attract, 0.25);
  v = blendVec(v, g.attract, 0.35);
  return {
    d: v.d * 0.55 + compass.dark * 0.45,
    w: v.w * 0.55 + compass.warm * 0.45,
    o: v.o * 0.55 + compass.organic * 0.45,
    e: v.e * 0.55 + compass.energy * 0.45,
    m: v.m * 0.7,
    s: v.s * 0.55 + compass.sad * 0.45,
  };
}

export function negativeDnaPenalty(t: Track): number {
  const d = getTasteDNA();
  if (d.negative.strength < 0.05) return 0;
  let p = 0;
  const a = d.negative.artists[t.artist.toLowerCase()];
  if (a) p += a * 2.2;
  const e = d.negative.emotions[t.emotion];
  if (e) p += e * 1.1;
...
export function recordSkip(trackId: string, progress: number) {
  ...
  if (progress < 0.08) kind = "instant";
  else if (progress < 0.35) kind = "early";
  else if (progress < 0.85) kind = "late";
  d.skips = [{ trackId, kind, at: Date.now() }, ...d.skips].slice(0, 40);
  const t = TRACKS.find((x) => x.id === trackId);
  if (t && (kind === "instant" || kind === "early")) {
    const a = t.artist.toLowerCase();
    d.negative.artists[a] = Math.min(
      1,
      (d.negative.artists[a] || 0) + (kind === "instant" ? 0.25 : 0.12)
    );
...
export function dnaSnapshot(fb: FB, compass: Compass) {
  ...
  return {
    longTermSamples: d.longTerm.sampleCount,
    mediumSamples: d.medium.sampleCount,
    sessionSamples: d.session.sampleCount,
    negativeStrength: Number(d.negative.strength.toFixed(2)),
    negativeArtists: Object.keys(d.negative.artists).slice(0, 6),
    recentSkips: d.skips.slice(0, 5),
    composite: compositeAttract(fb, compass),
  };
}
