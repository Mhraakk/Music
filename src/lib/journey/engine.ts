...
import { graph, keyOf, emotionalDist, recommend, type Compass, type FB } from "@/lib/engine";
...
export function transitionScore(a: Track, b: Track, role: JourneyRole): number {
  ...
  const obscurityStep =
    role === "deepen" || role === "peak" ? b.obscurity * 0.3 : b.obscurity * 0.1;
...
function roleReason(role: JourneyRole, t: Track): string {
  switch (role) {
    case "open":
      return `Opens the room · ${t.why}`;
    case "bridge":
      return `Bridge texture · ${t.why}`;
    case "lift":
      return `Lifts energy without breaking mood · ${t.emotion}`;
    case "deepen":
      return `Goes further in · obscurity ${(t.obscurity * 100).toFixed(0)}%`;
    case "peak":
      return `Emotional peak · ${t.why}`;
    case "land":
      return `Lands the arc · ${t.why}`;
  }
}
...
export function buildJourneyPlan(c: Compass, fb: FB, depth: number, length = 6): JourneyPlan {
  ...
  for (let i = 0; i < roles.length; i++) {
    ...
    path.push({
      t: best,
      role,
      chapter: ROLE_CHAPTER[role],
      reason: roleReason(role, best),
      transitionScore: prev ? transitionScore(prev, best, role) : 1,
      e: best.v.e,
    });
    prev = best;
  }
  ...
  const totalTransition =
    path.length > 1
      ? path.slice(1).reduce((s, p) => s + p.transitionScore, 0) / (path.length - 1)
      : 1;
  const session = getSession();
  const arc =
    depth > 0.7
      ? "Deep discovery arc"
      : session.temperature > 0.65
        ? "Exploratory arc"
        : "Centered emotional arc";
