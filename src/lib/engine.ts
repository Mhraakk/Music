...
function feedbackPenalty(f: FE | undefined): { hard: boolean; soft: number; note: string } {
  ...
  // ONLY "never" is hard exclude. Everything else is soft so catalog stays open.
  if (f.kind === "dislike") {
    if (f.reason === "never") return { hard: true, soft: -100, note: "hard veto" };
    return { hard: false, soft: -3.2, note: `reject:${f.reason || "dislike"}` };
  }
...
function scoreTrack(t: Track, tg: Vec, fb: FB, depth: number, seed: number): Scored {
  ...
  // Prefer UNRATED tracks so after like/dislike the list keeps filling with fresh songs
  if (!f) s += 1.8;
  else if (f.kind === "more" || f.kind === "like") s -= 0.4; // already taught — demote slightly so others surface
  else if (f.kind === "dislike" || f.kind === "less" || f.kind === "heard") s -= 0.6;

  const recentIdx = recentRecIds.indexOf(t.id);
  if (recentIdx >= 0) {
    s -= 0.7 + (RECENT_CAP - recentIdx) * 0.06;
  }
...
/** Never returns empty. Prefer unrated tracks after feedback. */
export function recommend(c: Compass, fb: FB, depth: number) {
  try {
    ...
    log("recommend()", { liked: g.liked, hated: g.hated, hardVetoes: hardCount, catalog: TRACKS.length, seed: sessionSeed });

    const scored = TRACKS.map((t) => scoreTrack(t, tg, fb, depth, sessionSeed)).sort((a, b) => b.s - a.s);

    // Prefer unrated first for "fresh songs after feedback"
    ...
    if (candidates.length === 0) {
      return emergencyFill(8, "Rotating full catalog — room never empty.", "full-rotate", g);
    }

    let items = diversify(candidates, 8);
    if (items.length < 4) {
      // Force more from catalog ignoring mild penalties
      const more = diversify(scored.filter((x) => x.s > -90), 8);
      if (more.length > items.length) items = more;
    }
    if (items.length === 0) {
      return emergencyFill(8, "Emergency catalog fill.", "emergency", g);
    }
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (path.length >= 1) {
        const alt = scored.find(
          (x) => x.s > -1e7 && !used.has(x.t.id) && x.t.artist.toLowerCase() !== pick.t.artist.toLowerCase()
        );
